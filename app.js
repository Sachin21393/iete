...
var level1=['the further','sadie','ronald defeo jr','samara morgan']
var level2=['derek jeter','mohali','hector mercedes','birmingham','steffi graf']
var level3=['midas','charlotte tannhaus','arnold weber']
      ...
      user_database.findOne({ email: email }, (err, data) => {
        try {
          ...
          if (data.password === password) {
            res.status(200).render("level1q1.pug");
          } else {
            res.send("invalid");
          }
        } catch {
...
app.get("/prize", (req, res) => {
  const params = {};
  res.status(200).render("prizes.pug", params);
});
